...
export default function EditTaskForm({ onTaskEdited, initialData, projectId, taskId }: TaskFormProps) {
  ...
  const onSubmit: SubmitHandler<TaskData> = async (data) => {
    setApiError('');
    try {
      await api.patch(`/tasks/${taskId}`, { ...data, projectId });
      onTaskEdited();
    } catch (err: any) {
  ...
  return (
    <div style={{ padding: '15px', marginBottom: '20px'}}>
      <h3 style={{ marginTop: 0 }}>Editing Task</h3>
